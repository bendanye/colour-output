...
def get_matching_rule(line: str, rules: List[Rule], extra_args=None):
    ...
    for rule in rules:
        if "contains " in rule.criteria:
            criteria = rule.criteria.replace("contains ", "")
            if re.search(criteria, line):
                return rule

        if "timestamp" in rule.criteria:
            ...
            rule_match = re.search(r"\$\{(\w+)\}", rule.criteria)
            if not rule_match:
            ...
            if not specified_date:
                raise ValueError(
                    f"Date key '{date_key}' not found in the date dictionary"
                )
            ...
            match = re.search(pattern, rule.criteria)
            if not match:
